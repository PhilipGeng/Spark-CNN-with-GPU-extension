...
/**
  ...
  */
class GPUCNNBatch(bs:Int) extends utility with Serializable{
  val batchSize = bs
  ...
  var data: Pointer = createDevicePointer(Array.fill[Float](in_h*in_w*batchSize){1f})
  var label: Pointer = createDevicePointer(Array.fill[Float](10*batchSize){1f})
  ...
  def networkTrain(data_host:Array[Float],label_host:Array[Int]): Unit ={
    var data_h = data_host
    var l_arr = label_host.map { x =>
      var arr = Array(0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f)
      arr(x) = 1f
      arr
    }.flatten
    cudaMemcpy(data,Pointer.to(data_h),data_h.length*Sizeof.FLOAT,cudaMemcpyHostToDevice)
